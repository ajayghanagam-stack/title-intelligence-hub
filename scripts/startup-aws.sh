...
# Uses Fargate (no EC2 instance needed).
...
echo -e "${BLUE}[1/3] Starting RDS instance...${NC}"
...
# Step 2: Wait for RDS to be ready
echo ""
echo -e "${BLUE}[2/3] Waiting for RDS to be ready...${NC}"
...
# Step 3: Scale ECS Fargate service back up
echo ""
echo -e "${BLUE}[3/3] Starting ECS Fargate tasks...${NC}"

# Check if service exists
SERVICE_STATUS=$(aws ecs describe-services --cluster ${APP_NAME}-cluster --services ${APP_NAME}-service --region ${AWS_REGION} --query 'services[0].status' --output text 2>/dev/null || echo "MISSING")
...
if [ "$SERVICE_STATUS" = "ACTIVE" ]; then
    ...
    echo -e "${RED}  ECS service not found or inactive (status: ${SERVICE_STATUS})${NC}"
    echo -e "${YELLOW}  You may need to recreate the service. See aws-deploy-part2.sh${NC}"
fi
